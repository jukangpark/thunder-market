...
const CheckBtn = styled.div<BtnColor>`
  width: 24px;
  height: 24px;
  border: 1px solid ${(props) => props.AllChecked ? "rgb(255, 255, 255)" : "rgb(204, 204, 204)"};
  position: relative;
  transition: 0.2s;
  background-color: ${(props) => props.AllChecked ? "rgb(247, 47, 51)" : "rgb(255, 255, 255)"};
  /* ::after {
      content: "";
      position: absolute;
      bottom: 5px;
  ...
  ::before {
    ...
    bottom: 5px;
    left: 2px;
    border: 2px solid ${(props) => props.AllChecked ? "rgb(255, 255, 255)" : "rgb(204, 204, 204)"};;
    border-left: 0;
    border-top: 0;
    width: 8px;
    height: 14px;
...
const CheckBtn2 = styled.div<BtnColor2>`
  width: 24px;
  height: 24px;
  border: 1px solid ${(props) => props.checked ? "rgb(255, 255, 255)" : "rgb(204, 204, 204)"};
  position: relative;
  transition: 0.2s;
  background-color: ${(props) => props.checked ? "rgb(247, 47, 51)" : "rgb(255, 255, 255)"};
  ::before {
    content: "";
    position: absolute;
    bottom: 5px;
    left: 2px;
    border: 2px solid ${(props) => props.checked ? "rgb(255, 255, 255)" : "rgb(204, 204, 204)"};;
    border-left: 0;
    border-top: 0;
    width: 8px;
    height: 14px;
          ...
          {favorites?.map((favorite) => (
            <ProductContainer key={favorite._id}>
              <Link to={`/product/${favorite._id}`}>
                ...
                <ProductImgBox>
                  <ProductImage imageUrl={`${favorite.imageUrl.replaceAll("\\", "/")}`} />
                </ProductImgBox>
                <InfoContainer>
                  <TitleAndPrice>
