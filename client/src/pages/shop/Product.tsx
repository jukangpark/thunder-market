...
const Product = () => {
  ...
  const lowCompare = (price: string) => {
    return (a:any, b:any) => (a[price] > b[price] ? 1 : (a[price] < b[price] ? -1 : 0))
  }
  const lowPrice = () => { 
  ...
  const popularityCompare = (views: string) => {
    return (a:any, b:any) => (a[views] < b[views] ? 1 : (a[views] > b[views] ? -1 : 0))
  }
  const popularity = () => {
        ...
        <GrossBox>
          <ProductAmount>
            ...
            <Sort>
              ...
              <Link to={`/shop/${user?._id}/products`} onClick={popularity} >인기순</Link>
              <Link to={`/shop/${user?._id}/products`} onClick={lowPrice}>저가순</Link>
              <Link to={`/shop/${user?._id}/products`} onClick={highPrice}>고가순</Link>
            </Sort>
          </ProductAmount>
        </GrossBox>
        <ProductsWrapper>
